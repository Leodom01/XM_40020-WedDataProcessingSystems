...
class FactChecking:
    ...
    def query_entity_properties(self, entity_id):
        """
    ...
    def get_most_similar_property(self, query, properties):
        """
        ...
        property_embeddings = self.embed(list(properties.keys()))
    ...
    def check_relationship(self, entity_id, relationship, subject_id):
        """
        ...
        properties = self.query_entity_properties(entity_id)
        most_similar_property, values = self.get_most_similar_property(relationship, properties)
        print(f"most_similar_property = {most_similar_property}")
        subject_is_entity = self.is_entity(subject_id)
        ...
        for value in values:
            if subject_is_entity:
                ...
                if self.is_entity(value):
                    entity_label = self.get_entity_label(value)
