...
def download_tacred(root_path=default_root_path):
    ...
    if not os.path.exists(os.path.join(root_path, 'benchmark/tacred')):
        ...
        logging.info('Due to copyright limits, we only provide rel2id for TACRED. Please download TACRED manually and convert the data to sdbhsdh format if needed.')
